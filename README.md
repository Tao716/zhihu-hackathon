...
用一份脱敏的真实会议纪要，完整展示「讨论原文 → 决策与任务 → 修正负责人 / 日期 → 人工确认 → 导出清单」。重点说明 AI 如何呈现未确定的信息，以及团队如何保留最后的判断权。

内置示例适合讲解交互；展示真实 AI 能力时，应配置模型并使用真实调用结果。若展示节省时间或准确率，请使用实际测量记录，并明确样本与评估方式。
